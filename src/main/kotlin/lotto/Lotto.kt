...
class Lotto(private val numbers: List<Int>) {
    ...
    init {
        require(numbers.size == LOTTO_SIZE) { "[ERROR] Lotto must contain exactly $LOTTO_SIZE numbers." }
        require(numbers.toSet().size == LOTTO_SIZE) { "[ERROR] Lotto numbers must be unique." }
        require(numbers.all { it in NUMBER_RANGE }) { "[ERROR] Lotto numbers must be between ${NUMBER_RANGE.first} and ${NUMBER_RANGE.last}." }
    }
