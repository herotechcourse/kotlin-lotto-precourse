...
object Validator {
    ...
    private const val ERROR_SUFFIX = ", please try again."
    ...
    fun checkNumberFormat(input: String) {
        require(input.matches(NUMERIC)) { errorMessage("Input must be a valid number") }
    }
    ...
    fun checkNumbersFormat(input: String) {
        require(input.matches(COMMA_SEPARATED)) { errorMessage("Input must be comma-separated") }
        require(input.matches(COMMA_SEPARATED_NUMBERS)) { errorMessage("Input must be comma-separated numbers") }
    }
    ...
    fun checkPurchaseAmountRules(value: Int) {
        require(value > 0) { errorMessage("Purchase amount must be greater than 0") }
        require(value % 1000 == 0) { errorMessage("Purchase amount must be a multiple of 1000") }
    }
    ...
    fun checkBonusNumberRules(value: Int, winningNumbers: List<Int>) {
        require(!winningNumbers.contains(value)) { errorMessage("Bonus number must not be in winning numbers") }
    ...
    fun checkLottoNumberRules(value: Int) {
        require(value > 0) { errorMessage("Lotto number must be greater than 0") }
        require(value in 1..45) { errorMessage("Lotto number must be between 1 and 45") }
    }
    ...
    fun checkLottoNumbersRules(value: List<Int>) {
        require(value.size == 6) { errorMessage("Lotto numbers must contain exactly 6 numbers") }
        require(value.toSet().size == 6) { errorMessage("Lotto numbers must not contain duplicates") }
        value.forEach { checkLottoNumberRules(it) }
    }

    private fun errorMessage(message: String) = "$ERROR_PREFIX $message$ERROR_SUFFIX"
}
