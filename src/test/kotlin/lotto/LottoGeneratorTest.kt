...
class LottoGeneratorTest {
    ...
    fun `throws error when amount is not divisible by 1000`() {
        assertThatThrownBy {
            LottoGenerator.countFromAmount(1550)
        }.isInstanceOf(IllegalArgumentException::class.java)
            .hasMessageContaining("divisible by 1,000")
    }

    @Test
    fun `returns correct ticket count based on amount`() {
        assertThat(LottoGenerator.countFromAmount(8000)).isEqualTo(8)
        assertThat(LottoGenerator.countFromAmount(3000)).isEqualTo(3)
    }
