...
class ApplicationTest : NsTest() {
    ...
    @Test
    fun `duplicate winning numbers test`() {
        assertSimpleTest {
            runException("1000", "1,2,2,4,5,6")
            assertThat(output()).contains("[ERROR] Winning numbers must be unique.")
        }
    }

    @Test
    fun `second rank win test`() {
        assertRandomUniqueNumbersInRangeTest(
            {
    ...
    @Test
    fun `invalid bonus number test`() {
        assertSimpleTest {
            runException("1000", "1,2,3,4,5,6", "1")
            assertThat(output()).contains("[ERROR] Bonus number must not be among winning numbers.")
        }
    }

    @Test
    fun `first rank win test`() {
        assertRandomUniqueNumbersInRangeTest(
            {
                run("1000", "1,2,3,4,5,6", "7")
                assertThat(output()).contains(
        ...
        assertRandomUniqueNumbersInRangeTest(
            {
                ...
                assertThat(output()).contains(
                    ...
                    "5 Matches + Bonus Ball (30,000,000 KRW) – 0 tickets",
                    "6 Matches (2,000,000,000 KRW) – 1 tickets",
                    "Total return rate is 20000550.0%.",
                )
            },
            listOf(1, 2, 3, 4, 5, 6),
            listOf(1, 3, 5, 14, 22, 45),
