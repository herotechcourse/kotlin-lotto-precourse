...
class WinningLottoTest {
    ...
    private val sut = WinLotto(Lotto(winningNumbers), 7)
    ...
    inner class ValidationTest {
        ...
        fun `throws an exception when bonus number is not in valid range`(invalidBonus: Int) {
            // Act
            // Assert
            assertThatThrownBy { WinLotto(winningLotto, invalidBonus) }
        ...
        fun `throws an exception when bonus number is included in winning numbers`() {
            ...
            // Act
            // Assert
            assertThatThrownBy { WinLotto(winningLotto, duplicateBonus) }
                .isInstanceOf(IllegalArgumentException::class.java)
