...
class LottoEvaluatorTest {
    ...
    fun `compareTickets should return empty map when no tickets provided`() {
        val results = evaluator.compareTickets(emptyList(), winningNumbers, bonusNumber)
    ...
    private fun testScenario(scenario: TestScenario) {
        val results = evaluator.compareTickets(scenario.tickets, winningNumbers, bonusNumber)
